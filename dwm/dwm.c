...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		//drw_rect(drw, m->ww - tw - sp, bh - 4, TEXTW(stext) - 36, 2, 1, 0);
	}
...
setup(void)
{
	...
	cursor[CurResize] = drw_cur_create(drw, XC_plus);
...
//		strcpy(stext, " Dynamic Window Manager - "VERSION );
		strcpy(stext, " I use rch BTW ");
	drawbar(selmon);
}
